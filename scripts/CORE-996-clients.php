...
//    $nbsp = "<br />";
    ...
    $sql = "SELECT id, username, email, phone, phone_ext, alt_phone, alt_phone_ext, alt_phone_2, alt_phone_2_ext FROM users
            WHERE 
            deleted = 0 AND email NOT LIKE '%echosigntest%' AND email NOT LIKE '%devtest%' AND email NOT LIKE '%importgenius%' AND email NOT LIKE '%codeninja%' AND email != 'ryang600@yahoo.com';
            ;";
    echo $sql.$nbsp.$nbsp;
    ...
    if ($num > 0) {
        while ($row = mysql_fetch_assoc($que)) {
            ...
            $updatesql = "UPDATE users SET phone = '{$phone}', alt_phone = '{$alt_phone}', alt_phone_2 = '{$alt_phone_2}' WHERE id = {$id}";
